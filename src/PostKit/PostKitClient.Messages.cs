...
{
    private const int MaxOutboundMessageCount = 500;
    private const int MaxOutboundMessageSearchWindow = 10_000;
    private const int MaxOutboundMessageMetadataNameLength = 20;
    private const int MaxOutboundMessageMetadataValueLength = 80;

    public Task<Result<OutboundMessagePage>> SearchOutboundMessagesAsync(MessageStream messageStream, int count = MaxOutboundMessageCount, int offset = 0, OutboundMessageQuery? query = null, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<Result<OutboundMessagePage>> SearchOutboundMessagesAsync(string messageStream, int count = MaxOutboundMessageCount, int offset = 0, OutboundMessageQuery? query = null, CancellationToken cancellationToken = default)
    {
        if (messageStream is null)
            throw new ArgumentNullException(nameof(messageStream), "The message stream ID cannot be null.");

        var validationError = ValidateOutboundMessageQuery(messageStream, count, offset, query);
        ...
        try
        {
            ...
            response = await postmark.GetAsync<OutboundSearchModel>(endpoint, cancellationToken);
        }
    ...
    public async Task<Result<OutboundMessageDetails>> GetOutboundMessageDetailsAsync(Guid messageId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            response = await postmark.GetAsync<OutboundMessageDetailsModel>($"/messages/outbound/{messageId:D}/details", cancellationToken);
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            throw;
    ...
    public async Task<Result<OutboundMessageDump>> GetOutboundMessageDumpAsync(Guid messageId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            response = await postmark.GetAsync<OutboundMessageDumpModel>($"/messages/outbound/{messageId:D}/dump", cancellationToken);
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            throw;
    ...
    private static string? ValidateOutboundMessageQuery(string messageStream, int count, int offset, OutboundMessageQuery? query)
    {
        ...
        if (!messageStream
            ...
            return ValidationExtensions.FormatMessageStreamIdValidationMessage(messageStream, "The outbound message query message stream");
        ...
        if (query.Recipient is not null && string.IsNullOrWhiteSpace(query.Recipient.Address))
            return FormatEmptyOutboundMessageQueryFilterMessage("The outbound message query recipient filter", nameof(OutboundMessageQuery.Recipient), query.Recipient.Address);

        if (query.FromEmail is not null && string.IsNullOrWhiteSpace(query.FromEmail.Address))
            return FormatEmptyOutboundMessageQueryFilterMessage("The outbound message query from-email filter", nameof(OutboundMessageQuery.FromEmail), query.FromEmail.Address);

        if (query.Tag is not null && string.IsNullOrWhiteSpace(query.Tag))
            return FormatEmptyOutboundMessageQueryFilterMessage("The outbound message query tag filter", nameof(OutboundMessageQuery.Tag), query.Tag);
        ...
        if (query.Subject is not null && string.IsNullOrWhiteSpace(query.Subject))
            return FormatEmptyOutboundMessageQueryFilterMessage("The outbound message query subject filter", nameof(OutboundMessageQuery.Subject), query.Subject);
    ...
    private static string? ValidateOutboundMessageMetadataFilter(OutboundMessageMetadataFilter? metadata)
    {
        if (metadata is null)
            return null;
        ...
        if (metadata.Name.Length > MaxOutboundMessageMetadataNameLength)
            return $"The outbound message query metadata filter name must not exceed {MaxOutboundMessageMetadataNameLength} characters. Actual length: {metadata.Name.Length}.";

        if (char.IsWhiteSpace(metadata.Name[0]))
            return $"The outbound message query metadata filter name is invalid. The metadata name is required, must not exceed {MaxOutboundMessageMetadataNameLength} characters, and cannot start or end with whitespace. Invalid leading whitespace {ValidationExtensions.FormatCharacter(metadata.Name[0])} at index 0.";

        if (char.IsWhiteSpace(metadata.Name[^1]))
            return $"The outbound message query metadata filter name is invalid. The metadata name is required, must not exceed {MaxOutboundMessageMetadataNameLength} characters, and cannot start or end with whitespace. Invalid trailing whitespace {ValidationExtensions.FormatCharacter(metadata.Name[^1])} at index {metadata.Name.Length - 1}.";
        ...
        if (metadata.Value.Length > MaxOutboundMessageMetadataValueLength)
            return $"The outbound message query metadata filter value must not exceed {MaxOutboundMessageMetadataValueLength} characters. Actual length: {metadata.Value.Length}.";

        return null;
    }

    private static string FormatEmptyOutboundMessageQueryFilterMessage(string subject, string propertyName, string? value)
    {
    ...
    private static Result<OutboundMessage> CreateOutboundMessage(OutboundMessageModel response)
    {
        ...
        var message = new OutboundMessage(
            outboundMessageCore.Tag,
            outboundMessageCore.MessageId,
            outboundMessageCore.MessageStream,
            outboundMessageCore.To,
            outboundMessageCore.Cc,
            outboundMessageCore.Bcc,
            outboundMessageCore.Recipients,
            outboundMessageCore.ReceivedAt,
            outboundMessageCore.From,
            outboundMessageCore.Subject,
            outboundMessageCore.Attachments,
            outboundMessageCore.Status,
            outboundMessageCore.TrackOpens,
            outboundMessageCore.TrackLinks,
            outboundMessageCore.Metadata,
            outboundMessageCore.Sandboxed
        );
    ...
    private static Result<OutboundMessageDetails> CreateOutboundMessageDetails(OutboundMessageDetailsModel response)
    {
        ...
        var messageDetails = new OutboundMessageDetails(
            outboundMessageCore.Tag,
            outboundMessageCore.MessageId,
            outboundMessageCore.MessageStream,
            outboundMessageCore.To,
    ...
    private static Result<OutboundMessageCore> CreateOutboundMessageCore(OutboundMessageModel response)
    {
        ...
        return Result.Success(new OutboundMessageCore(
                response.Tag,
                messageId,
                ...
                bcc,
                recipients,
                response.ReceivedAt.Value,
                response.From,
                response.Subject,
                attachments,
                status,
                response.TrackOpens.Value,
                trackLinks,
                metadata,
                response.Sandboxed.Value
            )
        );
    }
    ...
    private static Result<IReadOnlyDictionary<string, string>> CreateOutboundMessageMetadata(IReadOnlyDictionary<string, string?>? response)
    {
        ...
        foreach (var entry in response)
        {
            ...
            if (ValidationExtensions.TryGetExistingKey(metadata, entry.Key, out var existingKey))
                return Result.Failure<IReadOnlyDictionary<string, string>>($"Metadata returned from the Postmark Messages API contained duplicate names when compared case-insensitively. Metadata name '{entry.Key}' duplicates '{existingKey}'.");
    ...
    private static string GetOutboundMessageStatusQueryValue(OutboundMessageStatus status)
    {
        return status switch
        {
            ...
            _ => throw new NotImplementedException($"Outbound message status enum value of '{nameof(OutboundMessageStatus)}.{status}' has not been implemented. Please open an issue in the PostKit repository (https://github.com/jscarle/PostKit/issues)."),
        };
    }
