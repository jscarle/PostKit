...
public static class PostKitExtensions
{
    ...
    public static IServiceCollection AddPostKit(this IServiceCollection services)
    {
        ...
        return AddDefaultPostKitRegistration(
            services,
            ConfigurationSectionName,
            optionsBuilder => optionsBuilder.Configure<IConfiguration>((options, configuration) =>
                configuration.GetSection(ConfigurationSectionName).Bind(options)));
    }

    /// <summary>
    /// Registers the default (non-keyed) PostKit services using configuration from the <c>PostKit</c> section
    /// of the provided configuration root.
    /// </summary>
    /// <param name="services">The service collection to configure.</param>
    ...
    /// <summary>
    /// Registers the default (non-keyed) PostKit services using the provided configuration section.
    /// </summary>
    /// <param name="services">The service collection to configure.</param>
    /// <param name="configurationSection">The configuration section to bind options from.</param>
    /// <exception cref="InvalidOperationException">
    /// Thrown when the provided configuration section cannot be found.
    /// </exception>
    /// <returns>The same <paramref name="services"/> instance so calls can be chained.</returns>
    [UsedImplicitly]
    public static IServiceCollection AddPostKit(this IServiceCollection services, IConfigurationSection configurationSection)
    {
        ...
        return AddDefaultPostKitRegistration(
            services,
            GetOptionsName(configurationSection),
            optionsBuilder => optionsBuilder.Configure(configurationSection.Bind));
    }

    /// <summary>
    /// Registers PostKit services either as default (non-keyed) services or as keyed services.
    /// </summary>
    /// <param name="services">The service collection to configure.</param>
    /// <param name="serviceKey">
    /// The key used to resolve keyed services. When <see langword="null"/>, the default service registrations are rebound.
    /// </param>
    /// <param name="configurationKey">
    /// The configuration key to bind options from. When <paramref name="serviceKey"/> is provided and this value is omitted,
    /// the key is inferred from <paramref name="serviceKey"/>.
    /// </param>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="services"/> is <see langword="null"/>.
    /// </exception>
    /// <exception cref="InvalidOperationException">
    /// Thrown when a keyed registration cannot determine the configuration key.
    /// </exception>
    /// <returns>The same <paramref name="services"/> instance so calls can be chained.</returns>
    [UsedImplicitly]
        ...
        string? configurationKey = null)
    {
        ...
        return AddKeyedPostKitRegistration(
            services,
            serviceKey,
            resolvedSectionPath,
            optionsBuilder => optionsBuilder.Configure<IConfiguration>((options, configuration) =>
                configuration.GetSection(resolvedSectionPath).Bind(options)));
    }

    /// <summary>
    /// Registers keyed PostKit services using configuration from the <c>PostKit</c> section
    /// of the provided configuration root.
    /// </summary>
    /// <param name="services">The service collection to configure.</param>
        ...
        string? configurationKey = null)
    {
        ...
        var resolvedConfigurationKey = ResolveConfigurationKey(serviceKey, configurationKey);
        return services.AddKeyedPostKit(
            serviceKey,
            configuration.GetRequiredSection(ConfigurationSectionName)
                .GetRequiredSection(resolvedConfigurationKey));
    }

    /// <summary>
    /// Registers keyed PostKit services using the provided configuration section.
    /// </summary>
    /// <param name="services">The service collection to configure.</param>
    /// <param name="serviceKey">The key used to resolve the keyed services.</param>
    /// <param name="configurationSection">The configuration section to bind options from.</param>
    ...
    private static IServiceCollection AddDefaultPostKitRegistration(
        IServiceCollection services,
        string optionsName,
        Action<OptionsBuilder<PostKitOptions>> configureOptions)
    {
        ...
        services.Replace(ServiceDescriptor.Transient<IPostmarkClient>(sp =>
            sp.GetRequiredService<IPostmarkClientFactory>().Create()));

        services.Replace(ServiceDescriptor.Transient<IPostKitClient>(sp =>
            new PostKitClient(
                sp.GetRequiredService<IPostmarkClient>(),
                sp.GetRequiredService<ILogger<PostKitClient>>())));

        return services;
    }

    private static IServiceCollection AddKeyedPostKitRegistration(
        IServiceCollection services,
        object serviceKey,
        string optionsName,
        Action<OptionsBuilder<PostKitOptions>> configureOptions)
    {
        ...
        services.AddKeyedTransient<IPostmarkClient>(serviceKey, (sp, _) =>
            sp.GetRequiredService<IPostmarkClientFactory>().Create(optionsName));
    ...
    private static OptionsBuilder<PostKitOptions> AddValidatedOptions(IServiceCollection services, string optionsName)
    {
        ...
        optionsBuilder
            .Validate(
                options => !string.IsNullOrWhiteSpace(options.ServerApiToken),
    ...
    {
        return string.IsNullOrWhiteSpace(configurationSection.Path)
    ...
    private static void EnsureConfigurationSectionExists(IConfigurationSection configurationSection)
    {
        ...
        var sectionName = string.IsNullOrWhiteSpace(configurationSection.Path)
            ? configurationSection.Key
            : configurationSection.Path;
