...
public static class PostKitTemplateModelSerialization
{
    ...
    /// <summary>
    /// Gets or sets the default serializer options used when a template model is provided without explicit per-call serializer options.
    /// A defensive copy is returned and stored to avoid accidental external mutation of the global defaults.
    /// </summary>
    ...
    internal static JsonSerializerOptions Resolve(JsonSerializerOptions? serializerOptions)
    {
        return serializerOptions is null
            ? new JsonSerializerOptions(_defaultSerializerOptions)
