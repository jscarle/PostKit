...
internal sealed partial class PostKitClient
{
    ...
    public async Task<Result<SuppressionDump>> GetSuppressionsAsync(string messageStream, SuppressionQuery? query, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            response = await postmark.GetAsync<SuppressionDumpModel>(PostmarkTokenScope.Server, endpoint, cancellationToken);
        }
    ...
    private async Task<Result<SuppressionBatch>> ChangeSuppressionsAsync(string messageStream, IEnumerable<string> emailAddresses, string endpointSuffix, string operationName, Action<Exception> logException, Action<string, IError> logError,
        CancellationToken cancellationToken)
    {
        ...
        var validationError = ValidationExtensions.ValidateSuppressionMessageStream(messageStream, $"The suppression {operationName} message stream");
        if (validationError is not null)
        ...
        try
        {
            ...
            response = await postmark.PostAsync<SuppressionBatchRequestModel, SuppressionBatchModel>(PostmarkTokenScope.Server, endpoint, request, cancellationToken);
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            throw;
    ...
    private static string BuildSuppressionDumpEndpoint(string messageStream, SuppressionQuery? query)
    {
        ...
        if (query.ToDate.HasValue)
            parameters.Add($"todate={Uri.EscapeDataString(ValidationExtensions.FormatSuppressionQueryDate(query.ToDate.Value))}");

        if (query.FromDate.HasValue)
            parameters.Add($"fromdate={Uri.EscapeDataString(ValidationExtensions.FormatSuppressionQueryDate(query.FromDate.Value))}");
    ...
    private static string GetSuppressionReasonValue(SuppressionReason reason)
    {
        return reason switch
        {
            ...
            _ => throw new NotImplementedException(
                $"Suppression reason enum value of '{nameof(SuppressionReason)}.{reason}' has not been implemented. Please open an issue in the PostKit repository (https://github.com/jscarle/PostKit/issues).")
        };
    }
    ...
    private static string GetSuppressionOriginValue(SuppressionOrigin origin)
    {
        return origin switch
        {
            ...
            _ => throw new NotImplementedException(
                $"Suppression origin enum value of '{nameof(SuppressionOrigin)}.{origin}' has not been implemented. Please open an issue in the PostKit repository (https://github.com/jscarle/PostKit/issues).")
        };
    }
