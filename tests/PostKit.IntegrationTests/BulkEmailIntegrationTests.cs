...
public class BulkEmailIntegrationTests
{
    ...
    private async Task<BulkEmailStatusResponse> WaitForCompletionAsync(Guid bulkRequestId, CancellationToken cancellationToken)
    {
        BulkEmailStatusResponse? lastStatus = null;
    ...
    private static BulkEmailStatusResponse RequireBulkApi(Result<BulkEmailStatusResponse> result)
    {
        if (result.IsSuccess(out var response))
            return response;

        Assert.True(result.IsFailure(out var error, out BulkEmailStatusResponse? _), result.ToString());
